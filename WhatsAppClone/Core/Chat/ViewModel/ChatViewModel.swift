...
class ChatViewModel: ObservableObject {
    ...
    func finishRecording() async throws {
        audioRecorder?.stop()
        isRecording = false
       try await  updateMessageVoiceRecoreded()
    }
    ...
    private func updateMessageVoiceRecoreded() async throws {
...
//            self.messageGroups.append(MessageGroup(messages: messages, date: Date()))
            let groupedMessages = self.groupMessagByDate(message: messages)
    ...
    private func groupMessagByDate(message: [Message]) -> [MessageGroup] {
